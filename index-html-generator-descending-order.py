...
<header>
    ...
    for entry in sorted_entries:
        ...
        if not opts.include_hidden and entry.name.startswith('last-history'):
            continue

        if not opts.include_hidden and entry.name.startswith('gh-pages'):
            continue

        if not opts.include_hidden and entry.name.startswith('favicon.ico'):
            continue

        if not opts.include_hidden and entry.name.startswith('docs'):
            continue
