...
EXTERNAL_REPO_OWNER=${INPUT_EXTERNAL_REPO_OWNER}
EXTERNAL_REPOSITORY_NAME=${INPUT_EXTERNAL_REPO_NAME}
HOME_RUNNER_WORK=/home/runner/work/
...
if [[ ${INPUT_EXTERNAL_REPO_NAME} != '' ]]; then
    cd ${HOME_RUNNER_WORK}
    git clone https://github.com/${EXTERNAL_REPO_OWNER}/${EXTERNAL_REPOSITORY_NAME}.git --depth=1 --single-branch --branch ${{ INPUT_GH_PAGES }}
fi


if [[ ${INPUT_SUBFOLDER} != '' ]]; then
    INPUT_PLAYWRIGHT_HISTORY="${HOME_RUNNER_WORK}/${EXTERNAL_REPO_OWNER}/${INPUT_PLAYWRIGHT_HISTORY}/${INPUT_SUBFOLDER}"
